...
ByteStream TwoFish::f(ByteStream R0, ByteStream R1, unsigned int r) {
	...
	long long int f1 = (t0 + 2*t1 + k1)%(1LL<<32);
	ByteStream F0(f0);
	ByteStream F1(f1);
	vector<Byte> f;
	for(int i=0; i<4; i++)
...
ByteStream TwoFish::round(ByteStream x, unsigned int r, char type) {
	...
	vector<Byte> ans;
	for(int i=0; i<4; i++)
		ans.push_back(y0.getByte(i));
	for(int i=0; i<4; i++)
		ans.push_back(y1.getByte(i));
	for(int i=0; i<4; i++)
		ans.push_back(y2.getByte(i));
	for(int i=0; i<4; i++)
		ans.push_back(y3.getByte(i));
